...
def calculate_trading_metrics(trades_df, btc_price_df=None, time_filter=None, filter_time=None):
    ...
    if btc_price_df is not None and not btc_price_df.empty:
        if time_filter != "전체" and filter_time is not None:
            ...
            last_trade_time = closed_trades.sort_values('timestamp').iloc[-1][
                'timestamp' if 'exit_timestamp' not in closed_trades.columns else 'exit_timestamp']
    ...
    if 'exit_timestamp' in closed_trades.columns and 'timestamp' in closed_trades.columns:
        ...
        if not valid_timestamps.empty:
            holding_times = (valid_timestamps['exit_timestamp'] - valid_timestamps[
                'timestamp']).dt.total_seconds() / 3600  # 시간 단위
...
try:
    ...
    # 현재 BTC 가격
    current_btc_price = ai_analysis_df.iloc[0]['current_price'] if not ai_analysis_df.empty else btc_price_df.iloc[-1][
        'close']
    ...
    with position_cols[1]:
        ...
        position_color = "" if not has_open_position else (
            "positive" if current_position['action'] == 'long' else "negative")
        st.markdown(f"""
        <div class="metric-card" style="width: 100%">
    ...
    if not filtered_trades.empty:
        ...
        display_df = filtered_trades[
            ['timestamp', 'action', 'entry_price', 'exit_price', 'status', 'profit_loss']].copy()
    ...
    if has_open_position:
        ...
        with position_cols[1]:
            ...
            if isinstance(current_btc_price, (int, float)):
                ...
                price_color = "green" if (current_position['action'] == 'long' and price_diff > 0) or (
                            current_position['action'] == 'short' and price_diff < 0) else "red"
    ...
    if not ai_analysis_df.empty:
        ...
        with analysis_cols[0]:
            direction_color = "green" if latest_analysis['direction'] == 'LONG' else "red" if latest_analysis[
                                                                                                  'direction'] == 'SHORT' else "orange"
            st.markdown(f"""
            ### AI Analysis Summary
        ...
        with analysis_cols[1]:
            ...
            # 분석 내용 일부만 표시
            reasoning_preview = latest_analysis['reasoning'][:200] + "..." if len(
                latest_analysis['reasoning']) > 200 else latest_analysis['reasoning']
